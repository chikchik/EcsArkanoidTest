...
buildWrapperAndLink()
{
    ...
    ./build-windows.sh && (echo "$(tput setaf 2)BUILD COMPLETE, COPYING...$(tput setaf 3)" && cp -v $LIB_PATH $SERVER_PATH && cp -v $LIB_PATH $GAME_PATH && echo "$(tput setaf 2)DONE!" || echo "$(tput setaf 1)ERROR!") || echo "$(tput setaf 1)BUILD ERROR!"
}
